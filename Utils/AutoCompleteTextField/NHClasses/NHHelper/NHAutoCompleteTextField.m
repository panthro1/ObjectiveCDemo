...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.dataSourceDelegate respondsToSelector:@selector(didSelectRowAtIndexPath:)])
    {
        [self.dataSourceDelegate didSelectRowAtIndexPath:indexPath];
    }
    //[self endEditing:YES];
}
...
{
    NSInteger numberOfSource = [self.dataSourceDelegate numberOfSource];
    if(numberOfSource > 0)
    {
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if([self.dataSourceDelegate respondsToSelector:@selector(shouldReturn:)])
    {
        [self.dataSourceDelegate shouldReturn:textField];
    }
    return YES;
}
